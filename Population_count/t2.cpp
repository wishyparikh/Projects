...
bool test(int n) {
  ...
  //  make sure the 1st have is still alive and in (0,0)
  for(i=0; i<n/2; i++) {
    int r, c;
    r=-1; c=-1;
    if(!gw->whereis(i, r, c)) {
 ...
 * overall flow:
 *
 *     n birth operations in district 0,0
 *
 *     ids n/2 ... n-1 are killed
 *
 *     n-(n/2) birth operations in district 0,0
 *
 * (after this sequence, (0,0) should have ids 0..n-1 in order
 *   according to id management rules.)
 */
