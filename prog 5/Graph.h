...
class graph {
  ...
  public:
     ...
     */
    bool add_edge(const string &src, const string &dest, double weight=1.0) {
...
bool topo_sort(std::vector<int> &order) {
	...
	{		
        ...
        for( p=0; num_nodes() > p; p++ )
        {
				...
				for( bool result= que.empty(); result!= true; result = que.empty())
                {
                        p = que.front();
                        que.pop();
     ...
     */
	  ...
	  void findpaths(vector<vertex_label> & rpt, int src, int min) 
    {
      int u, v;
      std::queue<int> q;
      vector<edge> t;
      ...
      while(!q.empty()) {
        u = q.front();
        q.pop();
        t = (min) ? vertices[u].outgoing : vertices[u].incoming;
     ...
     bool dag_num_paths(vector<vertex_label> & rpt) 
	{
      ...
      for (int i = 0; i < num_nodes(); i++) {
        if (vertices[i].incoming.size() == 0) {
          findpaths(input, i, true);
        }

        if (vertices[i].outgoing.size() == 0) {
          findpaths(output, i, false);
        }
...
void allpaths(int u, int d, bool visited[], vector<string> &paths, vector<string> &path)
{
       ...
       for(edge &e : vertices[u].outgoing)
	{
		int v = e.vertex_id;
            if (!visited[v])
                allpaths(v, d, visited, paths, path);
    }
	}
    ...
    visited[u] = false;
	 ...
	 void Paths(int s, int d, vector<string> &paths)
{
    ...
    allpaths(s, d, visited, paths,path);
}
 ...
 bool enum_paths(int target, vector<string> &paths) {
      ...
      if(has_cycle() || target < 0 || target >= num_nodes())
		...
		Paths(vertices[0].id,target,paths);
