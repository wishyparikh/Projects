...
private:
	...
	{
		int id;
		vector<edge> outgoing;           // All outgoing vertex.   ----represents the edje.
		vector<edge> incoming;           //  All ingoing vertex.
		string name;                     // Name of the vertex----guess may be because of hashing.
	...
	unordered_map<string, int> _name2id;      // hashing.
	unordered_set<string> edges;             // hashing.
	vector<vertex> vertices;              // Vector of overall vertices. Like as in array of verticess.
...
public:
	...
	// by vertex-id.
	// Helpful struct for bfs.
	struct vertex_label
	{
		double dist;        // Finding shortest distance from source to all vertices.
		int pred;           //   
		char state;
		int npaths;
...
public:
	...
	*/
	...
	/*
	* func: add_edge------------------------------- UNDERSTAND THISS..-----------------1
	...
	*/
	bool add_edge(const string &src, const string &dest,
		...
		vertices[s_id].outgoing.push_back(edge(d_id, weight));              // Adding outgoing vertex in vector.
		vertices[d_id].incoming.push_back(edge(s_id, weight));             // Adding incoming vextex  in vector.
	...
	*/
	bool add_edge(const string &str) {
		...
		// The real helper function who does the most of the work.
				...
				if (report[v].state == UNDISCOVERED) {
					...
					if (u != src)
					{
						for (edge &e : vertices[v].incoming)
						{
							//printf("minDist = %d\n", minDist);
							if (minDist == -1)
								minDist = report[e.vertex_id].dist;

							if (minDist > report[e.vertex_id].dist)
								minDist = report[e.vertex_id].dist;
						}
					}
					else
						minDist = report[u].dist;

					//printf("v = %d, u = %d, minDist = %d\n", v, u, minDist);

					for (edge &e : vertices[v].incoming)
					{
						if (report[e.vertex_id].dist == minDist)
							report[v].npaths += report[e.vertex_id].npaths;
					}
