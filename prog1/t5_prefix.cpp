...
int main(int argc, char *argv[]){
  ...
  START("List::prefix COMPLETE test Suite (20/20 pts)"); 
  {
	int i;
	  ...
	  lsts[0] = arr2list(a, 6);
	  lsts[1] = lsts[0]->prefix(3);
	  TEST_RET(eq2array(lsts[1], left3, 3), 
	  ...
	  lsts[0] = arr2list(a, 6);
	  lsts[1] = lsts[0]->prefix( 5);
	  TEST_RET(eq2array(lsts[1], left5, 5), 
			"length-5 prefix test: LHS correct?",
			1, PtsPer);
	  TEST_RET(eq2array(lsts[0], right5, 1), 
			"length-5 prefix test: RHS correct?",
			1, PtsPer);
	  cleanup();
